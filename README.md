...
- [x]  tab_day.dart 에서 돼지갈비, 동치미 날짜입력이 안되는 문제
- [x]  본부장 대시보드- 매장별 기록 탭에 일주일, 한달, 일년, 전체를 설정
- [x]  매니저 tab_day ‘주간 보고’에서 유통기한을 눌렀을 때 수량조절 + - 버튼 생성
- [x]  본부장 대시보드에서 매장을 눌러서 내역을 살펴볼 때, 보고자가 누구인지 나오지 않음 → 생성
- [x]  본부장 대시보드에서 매장 상세 내역을 열었을 때, 최상단에 “2026-02-27 - xx점 상세 기록” 이라고 출력됨 → “2026년2월27일 만안점 마감내역” 으로 수정
- [x]  보고된 내용 마음대로 수정 불가
</aside>
...
- [x]  27일의 작업내용 이어서하기
- [x]  “”보고된 내용 마음대로 수정 불가 “” 추가기능 → 마감제출 시에, 모든 수정 버튼 잠금처리 (매장에서)
- [x]  본부장 대시보드에서 상단의 날짜선택 상자 수정 → 매장별 기록 탭에서는 필요없음, 전체매장 탭에서만 필요함
- [x]  휴대폰에 비밀번호 저장 및 자동 로그인 기능
- [x]  매니저 tab_report(기록열람) 에서도  본인 매장의 하루, 한달, 일년, 전체를 볼 수 있는 설정추가 / report_screen
    - [x]  tab_report 에서 ‘오전 근무 보고 상세 보기’ 모든 내용이 출력되지 않음.
- [ ]  (내용확인 후 작업) 매니저 tab_closing 에서 매출 내용이 tab_sales의 내용과 불일치 할 때, 알람 팝업을 띄운후, 강제 저장할 것인지.
- [x]  본부장 대시보드 → 매장별 기록 탭 → 날짜 표시 : 현재 x월 x일 → x년 x월 x일 로 표시
- [x]  본부장 대시보드 → 매장별 기록 → 매장 선택 중 ‘전체매장’ 추가
    - [x]  기록된 보고서가 없다고 뜸. 리스트
</aside>
